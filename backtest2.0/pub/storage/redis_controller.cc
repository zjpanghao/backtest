...
bool RedisController::AddHashElement(std::string& key, std::string& value){
    base_storage::DictionaryStorageEngine* redis_engine = GetRedisEngine();
    if( NULL == redis_engine ){
        return false;
    }
    return redis_engine->AddValue(key.c_str(), key.size(), value.c_str(), value.size());
}

bool RedisController::GetHashElement(std::string& key, std::string& value){
    bool r = false;
    base_storage::DictionaryStorageEngine* redis_engine = GetRedisEngine();
    if( NULL == redis_engine ){
        return false;
    ...
    size_t result_len = 0;
    r = redis_engine->GetValue(key.c_str(), key.size(), &str_result, &result_len);
    if( !r ){
        return false;
    }
